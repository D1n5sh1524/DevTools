...
class ToonTab(ttk.Frame):
    ...
    def json_to_toon(self, data, entity_name="items"):
        """
        Convert list[dict] JSON into TOON format.
        """
        ...
        if not isinstance(data, list):
            return str(data)
        if not data:
            return f"{entity_name}[0]{{}}:"
        if not all(isinstance(x, dict) for x in data):
            return str(data)
        columns = list(data[0].keys())
        lines = [
            f"{entity_name}[{len(data)}]{{{','.join(columns)}}}:"
        ]
        for row in data:
            values = []
            for col in columns:
    ...
    def fmt(self):
        ...
        try:
            data = json.loads(raw)
            toon = self.json_to_toon(data)
            self._write_output(toon, highlight=False)
            rows = len(data) if isinstance(data, list) else 1
            self._set_status(
                f"✓ Converted to TOON · {rows} records",
                STATUS_OK
            )
